...
def train(train_dataloader,
    ...
    for i in range(epoch):
        ...
        if local_rank == 0:
            # Best-val-loss checkpoint, tracked and overwritten every epoch
            # regardless of log_print_interval_epoch/model_save_interval_epoch
            # -- added 2026-08-24 so a run whose val loss plateaus/oscillates
            # well before the final epoch (see LTO_Darcy_resaug_normalized_dilated's
            # training log) still leaves behind its actual best checkpoint,
            # not just whichever numbered epoch happened to land last.
            # Purely additive: doesn't change the existing per-epoch/
            # print-interval behavior below, and every existing config
            # (including LNO's own runs) gets this for free.
            ...
            if (i + 1) % log_print_interval_epoch == 0:
                ...
                print("Epoch: {}\tLearning Rate :{}\tTrain Loss: {}\tVal Loss: {}\tBest Val Loss: {}".format(i+1, optimizer.state_dict()['param_groups'][0]['lr'], train_loss, val_loss, best_val_loss))
                logger.print("Epoch: {}\tLearning Rate :{}\tTrain Loss: {}\tVal Loss: {}\tBest Val Loss: {}".format(i+1, optimizer.state_dict()['param_groups'][0]['lr'], train_loss, val_loss, best_val_loss))
...
def train_time(train_dataloader,
    ...
    for i in range(epoch):
        ...
        if local_rank == 0:
            # Same best-checkpoint tracking as train() above, keyed on
            # val_loss_full (the metric that actually matters -- the full
            # rolled-out trajectory error, not the single-step loss).
            if val_loss_full < best_val_loss_full:
                best_val_loss_full = val_loss_full
                checker.save_best()
            ...
            if (i + 1) % log_print_interval_epoch == 0:
                ...
                print("Epoch: {}\tLearning Rate :{}\tTrain Loss Step: {}\tTrain Loss Full: {}\tVal Loss Step: {}\tVal Loss Full: {}\tBest Val Loss Full: {}".format(i+1, optimizer.state_dict()['param_groups'][0]['lr'], train_loss_step, train_loss_full, val_loss_step, val_loss_full, best_val_loss_full))
                logger.print("Epoch: {}\tLearning Rate :{}\tTrain Loss Step: {}\tTrain Loss Full: {}\tVal Loss Step: {}\tVal Loss Full: {}\tBest Val Loss Full: {}".format(i+1, optimizer.state_dict()['param_groups'][0]['lr'], train_loss_step, train_loss_full, val_loss_step, val_loss_full, best_val_loss_full))
